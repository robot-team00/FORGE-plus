...
#
#   bash scripts/sweep_jam_recovery.sh [CKPT] [EPISODES] [GRIPPER]
#
...
EP="${2:-25}"
GRIPPER="${3:-franka_panda}"
OUT=/workspace/jam_sweep
[ "$GRIPPER" = "robotiq_2f140" ] && OUT=/workspace/jam_sweep_rq
mkdir -p "$OUT"
...
for MODE in ours heuristic vision_llm press_harder none; do
    ...
    /workspace/.venv/bin/python scripts/eval_gear_jam.py \
        ...
        --gripper "$GRIPPER" --ckpt "$CKPT" > "$LOG" 2>&1
